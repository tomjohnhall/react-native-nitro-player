...
class PlaylistManager private constructor(private val context: Context) {
     ...
     */
    fun createPlaylist(name: String, description: String? = null, artwork: String? = null): String {
    ...
    fun deletePlaylist(playlistId: String): Boolean {
        val removed = synchronized(playlists) {
     ...
     */
    fun updatePlaylist(playlistId: String, name: String? = null, description: String? = null, artwork: String? = null): Boolean {
        val playlist = synchronized(playlists) {
            playlists[playlistId]
        } ?: return false
        
        synchronized(playlists) {
            playlists[playlistId] = playlist.copy(
     ...
     */
    fun addTrackToPlaylist(playlistId: String, track: TrackItem, index: Int? = null): Boolean {
        val playlist = synchronized(playlists) {
            playlists[playlistId]
        } ?: return false
     ...
     */
    fun addTracksToPlaylist(playlistId: String, tracks: List<TrackItem>, index: Int? = null): Boolean {
        val playlist = synchronized(playlists) {
            playlists[playlistId]
        } ?: return false
     ...
     */
    fun reorderTrackInPlaylist(playlistId: String, trackId: String, newIndex: Int): Boolean {
        val playlist = synchronized(playlists) {
            playlists[playlistId]
        } ?: return false
    ...
    fun getCurrentPlaylist(): Playlist? {
        return currentPlaylistId?.let { synchronized(playlists) { playlists[it] } }
    }
     ...
     */
    fun addPlaylistChangeListener(playlistId: String, listener: (Playlist, QueueOperation?) -> Unit): () -> Unit {
    ...
    private fun notifyPlaylistsChanged(operation: QueueOperation?) {
        val allPlaylists = synchronized(playlists) {
            playlists.values.toList()
        }
        listeners.forEach { it(allPlaylists, operation) }
    }
    
    private fun notifyPlaylistChanged(playlistId: String, operation: QueueOperation?) {
        val playlist = synchronized(playlists) {
            playlists[playlistId]
        } ?: return
                ...
                synchronized(playlists) {
                    ...
                    for (i in 0 until jsonArray.length()) {
                        ...
                        for (j in 0 until tracksArray.length()) {
                            ...
                            val artwork: Variant_NullType_String? = if (!artworkStr.isNullOrEmpty()) {
