...
class EqualizerCore {
    ...
    private let listenersQueue = DispatchQueue(label: "com.equalizer.listeners", attributes: .concurrent)
    ...
    func applyAudioMix(to playerItem: AVPlayerItem) {
        ...
        asset.loadValuesAsynchronously(forKeys: ["tracks"]) { [weak self] in
            ...
            if status == .failed {
                print("⚠️ EqualizerCore: Failed to load tracks key: \(error?.localizedDescription ?? "unknown")")
                return
            }
    ...
    func getCustomPresets() -> [EqualizerPreset] {
        guard let data = UserDefaults.standard.data(forKey: customPresetsKey),
              let presets = try? JSONDecoder().decode([String: [Double]].self, from: data) else {
            return []
        }
    ...
    func deleteCustomPreset(_ name: String) -> Bool {
        guard var presets: [String: [Double]] = {
            guard let data = UserDefaults.standard.data(forKey: customPresetsKey),
                  let existing = try? JSONDecoder().decode([String: [Double]].self, from: data) else {
                return nil
            }
            return existing
        }() else {
    ...
    func addOnPresetChangeListener(owner: AnyObject, _ callback: @escaping (Variant_NullType_String?) -> Void) {
        listenersQueue.async(flags: .barrier) { [weak self] in
            let box = WeakCallbackBox(owner: owner, callback: callback)
...
private func tapPrepareCallback(
    ...
    print("🎛️ EqualizerCore: Tap prepared - sampleRate: \(context.sampleRate), channels: \(context.channelCount)")
}
