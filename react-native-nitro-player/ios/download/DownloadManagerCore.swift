...
final class DownloadManagerCore: NSObject {
    ...
    private let queue = DispatchQueue(label: "com.nitroplayer.downloadManager", attributes: .concurrent)
    ...
    func downloadTrack(track: TrackItem, playlistId: String?) -> String {
        ...
        queue.async(flags: .barrier) {
            ...
            guard let url = URL(string: track.url) else {
                self.notifyStateChange(downloadId: downloadId, trackId: track.id, state: .failed, error: DownloadError(
                    code: "INVALID_URL",
            ...
            self.notifyStateChange(downloadId: downloadId, trackId: track.id, state: self.taskMetadata[downloadId]?.state ?? .pending, error: nil)
        }
    ...
    func syncDownloads() -> Int {
        ...
        print("🔄 DownloadManagerCore: syncDownloads completed - removed \(removedFromDb) orphaned records, freed \(bytesFreed) bytes")
    ...
    func addStateChangeCallback(_ callback: @escaping (String, String, DownloadState, DownloadError?) -> Void) {
        queue.async(flags: .barrier) {
    ...
    private func notifyStateChange(downloadId: String, trackId: String, state: DownloadState, error: DownloadError?) {
        DispatchQueue.main.async {
            ...
            if let pendingId = self.taskMetadata.first(where: { $0.value.state == .pending })?.key,
               let task = self.activeTasks[pendingId] {
                ...
                if let trackId = self.taskMetadata[pendingId]?.trackId {
                    self.notifyStateChange(downloadId: pendingId, trackId: trackId, state: .downloading, error: nil)
                }
            }
        }
    }
...
extension DownloadManagerCore: URLSessionDownloadDelegate {
    ...
    func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didFinishDownloadingTo location: URL) {
        ...
        print("🎯 DownloadManagerCore: Processing completion for downloadId=\(downloadId), trackId=\(trackId)")
        ...
        queue.async(flags: .barrier) {
            guard let destinationPath = destinationPath else {
                ...
                self.notifyStateChange(downloadId: downloadId, trackId: trackId, state: .failed, error: self.taskMetadata[downloadId]?.error)
                return
            }
            ...
            guard let track = self.trackMetadata[trackId] else {
                ...
                self.activeTasks.removeValue(forKey: downloadId)
                self.notifyStateChange(downloadId: downloadId, trackId: trackId, state: .completed, error: nil)
    ...
    func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didWriteData bytesWritten: Int64, totalBytesWritten: Int64, totalBytesExpectedToWrite: Int64) {
        ...
        queue.async(flags: .barrier) {
            ...
            self.taskMetadata[downloadId]?.totalBytes = totalBytesExpectedToWrite > 0 ? Double(totalBytesExpectedToWrite) : nil
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        guard let downloadTask = task as? URLSessionDownloadTask,
              let description = downloadTask.taskDescription else { return }
        ...
        queue.async(flags: .barrier) {
            ...
            if let autoRetry = self.config.autoRetry, autoRetry,
               ...
               retryCount < Int(self.config.maxRetryAttempts ?? 3) {
                ...
                self.notifyStateChange(downloadId: downloadId, trackId: trackId, state: .failed, error: downloadError)
            }
