...
final class DownloadDatabase {
    ...
    private let queue = DispatchQueue(label: "com.nitroplayer.downloadDatabase", attributes: .concurrent)
    ...
    func isTrackDownloaded(trackId: String) -> Bool {
        return queue.sync {
            ...
            if exists {
                ...
                print("❌ DownloadDatabase: Track \(trackId) record exists but file NOT found at \(record.localPath)")
            }
            return exists
        }
    }
    ...
    func getAllDownloadedTracks() -> [DownloadedTrack] {
        return queue.sync {
            print("🎯 DownloadDatabase: getAllDownloadedTracks called, have \(downloadedTracks.count) records")
    ...
    func getDownloadedPlaylist(playlistId: String) -> DownloadedPlaylist? {
        return queue.sync { () -> DownloadedPlaylist? in
            ...
            guard let playlistModel = PlaylistManager.shared.getPlaylist(playlistId: playlistId) else { return nil }
            ...
            return DownloadedPlaylist(
                ...
                totalSize: totalSize,
                downloadedAt: downloadedTracks.map { $0.downloadedAt }.min() ?? Date().timeIntervalSince1970,
    ...
    private func loadFromDisk() {
        ...
        if let tracksData = UserDefaults.standard.data(forKey: Self.downloadedTracksKey) {
            do {
                self.downloadedTracks = try JSONDecoder().decode([String: DownloadedTrackRecord].self, from: tracksData)
        ...
        if let playlistData = UserDefaults.standard.data(forKey: Self.playlistTracksKey) {
            do {
                let playlistTracksDict = try JSONDecoder().decode([String: [String]].self, from: playlistData)
                self.playlistTracks = playlistTracksDict.mapValues { Set($0) }
                print("✅ DownloadDatabase: Loaded \(self.playlistTracks.count) playlist associations from disk")
