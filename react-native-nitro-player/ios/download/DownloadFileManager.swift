...
final class DownloadFileManager {
    ...
    func saveDownloadedFile(from temporaryLocation: URL, trackId: String, storageLocation: StorageLocation, originalURL: String? = nil) -> String? {
        ...
        let destinationDirectory = storageLocation == .private ? privateDownloadsDirectory : publicDownloadsDirectory
    ...
    func getStorageInfo() -> DownloadStorageInfo {
        ...
        if let enumerator = fileManager.enumerator(at: privateDownloadsDirectory, includingPropertiesForKeys: [.fileSizeKey]) {
        ...
        if let enumerator = fileManager.enumerator(at: publicDownloadsDirectory, includingPropertiesForKeys: [.fileSizeKey]) {
            for case let fileURL as URL in enumerator {
                if let fileSize = try? fileURL.resourceValues(forKeys: [.fileSizeKey]).fileSize {
                    totalDownloadedSize += Int64(fileSize)
                    trackCount += 1
    ...
    func cleanupOrphanedFiles() -> Int64 {
        ...
        let downloadedTrackIds = Set(DownloadDatabase.shared.getAllDownloadedTracks().map { $0.trackId })
    ...
    func getLocalPath(for trackId: String) -> String? {
        ...
        let privateFiles = (try? fileManager.contentsOfDirectory(at: privateDownloadsDirectory, includingPropertiesForKeys: nil)) ?? []
        ...
        // Check public directory
        let publicFiles = (try? fileManager.contentsOfDirectory(at: publicDownloadsDirectory, includingPropertiesForKeys: nil)) ?? []
